...
In my time at [Agworld](https://agworld.com), I embarked on a long-lasting project to pack back some of the [technical, and design-debt](https://en.wikipedia.org/wiki/Technical_debt) that had built up in its core web product over a decade. Specifically focused on the way we handled styling, with a view to improve performance, legibility, accessibility and overall developer experience.
...
- Every page bundled up and served **4 (FOUR)** versions of the popular styling library [Bootstrap](https://getbootstrap.com/); which made the overall CSS bundle size enormous
...
- There were no clearly defined performance budgets, so we didn't know just how poorly we were doing
...
A colleague of mine mentioned this quote around the time that this project was kicking off, and it's certainly far easier than it sounds! It was extremely difficult to get a broad understanding of the breadth of the change that I needed to make, so I approached it by dividing work into high-level _themes_, and then those _themes_ into steps and stages that could be shipped incrementally.

I led the project over the course of 3 _sprints_ lasting 6 weeks each; with the assistance from members of a team dedicated to technical improvements.
...
Agworld's engineering team values a high shipping cadence, and spent significant resources improving this over the time I worked there, and while at one stage was shipping several times per day, and dozens of times per week; the core product's architecture forced a refreshed asset payload on a large number of those deployments; and when the payload is as large as this was, that could be extremely impactful on users - particularly those located in remote areas; so it was _extremely important_ that these assets were cut down to size.
...
The great majority of this payload was unused - meaning it had literally no impact on the final product. On some of the most heavily used pages of the site, over 95% of the served CSS was unused. This was due to the fact that all resources were bundled together, even if they were relevant to that particular page. So one page may be using Bootstrap 3, another with Bootstrap 4, and all of it was downloaded together.
...
In real-world numbers, the overall size of the bundle was ~430kb. Spltting this up into what was required for different pages meant potentially some only needed to download **~131kb**, and others **~162kb**. Any reduction here is welcome, but this represented one of the `most signifcant, technical savings to the company in the running of the core product`. This work was also volatile, so I along with help from the technical team put in significant testing time and effort to ensure high quality.

### Consolidating layout

The final step in this refactoring project was to consolidate the three, vastly different layouts into a single one.
...
I was particularly interested in completing this, as this would not only unblock further work I would go onto undertake around responsive layout, but also dramatically reduce the cognitive load and complexity facing developers working on new and existing features. A real win for the developer experience at Agworld.

#### Sandbox Resources

The main problem I faced here was determining how to ensure that legacy content and experiences still behaved as desired with a single layout.

<Figure caption="Uh oh!">
  <Picture
...
I decided to use a sandboxing strategy to isolate resources, layout, and content based on the different - what I termed - layout "contexts" identified in the audit shown above. Each of these served namespaced layout, resets, and resources required for content within it - effectively shielding it from other, potentially damaging influences.
